...
fn gen_completions<T: uucore::Args>(
    ...
    let all_utilities: Vec<_> = std::iter::once("hostname")
    ...
    let mut command = if utility == "hostname" {
        gen_hostname_app(util_map)
    } else {
...
fn gen_manpage<T: uucore::Args>(
    ...
    let command = if utility == "hostname" {
        gen_hostname_app(util_map)
    } else {
        util_map.get(utility).unwrap().1()
    };
...
fn gen_hostname_app<T: uucore::Args>(util_map: &UtilityMap<T>) -> Command {
